...
This file lives on the PR branch itself, so the code it describes is checked out
alongside it.

| | |
| --- | --- |
| Branch | `claude/polars-lazy-eval-review-pxn4li` |
| Reviewed head | `7a2341a` — the commit directly below this file |
| Base | `master` @ `6de8dbf` |

Every file:line reference below is against `7a2341a`. If commits have landed on
top since, re-derive them; the symbols named in each phase (`_jacknife`,
`_air_func`, `set(thresholds or y_score)`, …) are stable enough to grep for.
